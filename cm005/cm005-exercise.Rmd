...
```{r}
filter(gapminder, pop > 100e6)
```

2. Of those, only take data from Asia.
```{r}
filter(gapminder, pop > 100e6) %>% filter(continent == "Asia")
```
...
```{r}
filter(gapminder, continent == "Europe", gdpPercap > 10000) %>% select(- gdpPercap)
```
