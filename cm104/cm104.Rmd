...
```{r}
comm_fun <- function(name, commute) paste(name, " takes ", commute, " minutes to get to work.")
...
```{r}
icecream %>% 
  filter(map_lgl(flav, ~ "strawberry" %in% .x))
```
...
iris %>%
  group_by(Species) %>%
  nest() %>%
  {map(.$data, ~ lm(Sepal.Length ~ Sepal.Width, .x))} %>%
  map(broom::tidy) %>%
  str()
```
