...
![](cm007_files/figure-gfm/unnamed-chunk-14-2.png)<!-- -->

``` r
g + geom_point(alpha=0.2) + geom_smooth(method="lm")
```

![](cm007_files/figure-gfm/unnamed-chunk-14-3.png)<!-- -->
...
try piping the data frame into the `ggplot`
function.

``` r
gapminder %>% ggplot(aes(year, lifeExp, color=continent)) + geom_point() + geom_smooth(se=FALSE, method="lm")
```
...
``` r
gapminder %>% ggplot(aes(year, lifeExp, color=continent)) + geom_smooth(se=FALSE, method="lm") + geom_point()
```
...
though the data aren’t shown? Why or why
not?

``` r
gapminder %>% ggplot(aes(year, lifeExp, color=continent)) + geom_smooth(se=FALSE, method="lm")
```
...
``` r
gapminder %>% ggplot(aes(year, lifeExp)) + facet_wrap(~continent) + geom_smooth(se=FALSE)
```

    ## `geom_smooth()` using method = 'loess'

![](cm007_files/figure-gfm/unnamed-chunk-20-1.png)<!-- --> **Exercise
5**: Plot the population over time (year) using lines, so that each
country has its own line. Colour by `gdpPercap`. Add alpha transparency
to your
liking.

``` r
gapminder %>% ggplot(aes(year, pop, color=gdpPercap, group=country)) + geom_line(alpha=0.5)
```

![](cm007_files/figure-gfm/unnamed-chunk-21-1.png)<!-- -->

**Exercise 6**: Add points to the plot in Exercise
5.

``` r
gapminder %>% ggplot(aes(year, pop, color=gdpPercap, group=country)) + geom_line(alpha=0.5) + geom_point(alpha=0.5)
```
